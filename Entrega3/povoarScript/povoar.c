...
int main()
{
    ...
    int aux, aux2 = 0, aux3 = 0, aux4 = 0, players[MAX_PLAYERS], winners[MAX_MATCHS],
             ...
             positionTChessClub = 0, tournamentChessClub[MAX_TOURNAMENTS][MAX_PLAYERS_IN_TOURNAMENTS], finalTwinner[MAX_TOURNAMENTS];
    ...
    for (int i = 0; i < MAX_ACCOUNTS; i++)
    {
        ...
        if (aux > 2400)
        {
            fprintf(dest, "INSERT INTO Account VALUES (%d,\"%s\",\"%s\",\"%s\",%d,%d,%d);\n",
                    uniqueId[i + MAX_PLAYERS + MAX_WEBSITES], buff, buff2, "Grandmaster (GM)", aux, websites[rand() % MAX_WEBSITES], players[i]);
        }
        else if (aux > 2300)
        {
            fprintf(dest, "INSERT INTO Account VALUES (%d,\"%s\",\"%s\",\"%s\",%d,%d,%d);\n",
                    uniqueId[i + MAX_PLAYERS + MAX_WEBSITES], buff, buff2, "International Master (IM)", aux, websites[rand() % MAX_WEBSITES], players[i]);
        }
        else if (aux > 2200)
        {
            fprintf(dest, "INSERT INTO Account VALUES (%d,\"%s\",\"%s\",\"%s\",%d,%d,%d);\n",
                    uniqueId[i + MAX_PLAYERS + MAX_WEBSITES], buff, buff2, "FIDE Master (FM)", aux, websites[rand() % MAX_WEBSITES], players[i]);
        }
        else if (aux > 2000)
        {
            fprintf(dest, "INSERT INTO Account VALUES (%d,\"%s\",\"%s\",\"%s\",%d,%d,%d);\n",
                    uniqueId[i + MAX_PLAYERS + MAX_WEBSITES], buff, buff2, "Candidate Master (CM)", aux, websites[rand() % MAX_WEBSITES], players[i]);
        }
        else
        {
            fprintf(dest, "INSERT INTO Account VALUES (%d,\"%s\",\"%s\",%s,%d,%d,%d);\n",
                    uniqueId[i + MAX_PLAYERS + MAX_WEBSITES], buff, buff2, "NULL", aux, websites[rand() % MAX_WEBSITES], players[i]);
        }
    ...
    for (int i = MAX_MATCHS - ((MAX_PLAYERS_IN_TOURNAMENTS - 1) * MAX_TOURNAMENTS); i < MAX_MATCHS; i++)
    {
        ...
        else
        {
            fprintf(dest, "INSERT INTO Match VALUES (%d,\"%s%d\",'%s','%s','%s',%d,%d,\"%s\",%d,%d);\n",
                    uniqueId[i + MAX_PLAYERS + MAX_WEBSITES + MAX_ACCOUNTS + MAX_TOURNAMENTS], "Win at move ", aux4,
